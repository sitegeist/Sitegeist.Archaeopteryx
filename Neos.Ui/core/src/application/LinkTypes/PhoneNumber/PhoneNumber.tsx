...
export const PhoneNumber = makeLinkType<PhoneNumberLinkModel, PhoneNumberLinkOptions>(
	...
	({ createError }) => ({
		...
		useResolvedModel: (link: ILink) => {
			...
			return Process.error(createError(`Cannot handle href "${link.href}".`))
		},
		...
		Editor: ({
			...
			model: Nullable<PhoneNumberLinkModel>
			options: OptionalDeep<PhoneNumberLinkOptions>
		}) => {
			const defaultCountryCallingCode: string =
				model?.countryCallingCode ||
				(options?.defaultCountry
					? `+${getCountryCallingCode(options?.defaultCountry).toString()}`
					: `+${getCountryCallingCode(getCountries()[0]).toString()}`)
					...
					>
						<Field<string>
							...
							format={(value) => {
								...
								if (value === '' || value === undefined) {
									useForm().change(
										'linkTypeProps.Sitegeist_Archaeopteryx:PhoneNumber.countryCallingCode',
										areaCode
									)
								}
								return value
							...
							validate={(value) => {
								if (!value) {
									return i18n(
										'Sitegeist.Archaeopteryx:LinkTypes.PhoneNumber:countryCallingCode.validation.required'
									)
								}
							}}
						>
						...
						<Field<string>
							...
							validate={(value) => {
								if (!value) {
									return i18n(
										'Sitegeist.Archaeopteryx:LinkTypes.PhoneNumber:phoneNumber.validation.required'
									)
								}
								if (!checkRegex.test(value)) {
									return i18n(
										'Sitegeist.Archaeopteryx:LinkTypes.PhoneNumber:phoneNumber.validation.numbersOnly'
									)
								}
							}}
						>
							{({ input, meta }) => (
								<EditorEnvelope
									...
									editorOptions={{
										placeholder: i18n(
											'Sitegeist.Archaeopteryx:LinkTypes.PhoneNumber:phoneNumber.placeholder'
										),
									}}
									input={input}
									meta={meta}
